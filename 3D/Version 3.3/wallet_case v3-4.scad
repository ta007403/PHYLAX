...
// This one is for outer lock at 4 corner by Tar
m3_head_d        = 5.8;   // [5.5:0.1:7.0] mm, M3 COUNTERSUNK (flat head) screw head
...
module back_shell() {
    screw_boss_pad_d = shell_screw_csk_d + 2.4 + 3.1;
    ...
    difference() {
        union() {
            ...
            // This one is for 4 outer corner of hardware wallet
            for (p = mount_positions) {
                translate([p[0] + 2, p[1] + 2, -screw_boss_pad_h])
                    cylinder(d=screw_boss_pad_d - 1, h=screw_boss_pad_h + overlap2);
            }
            ...
            for (p = sw_support_positions) {
                translate([p[0], p[1], -sw_support_boss_h + overlap2 + 1])  // Revise by Tar
        ...
        // Add by Tar for install M3 hex nut for 4 corner as PCB pole
        
        for (p = mount_positions) {
            translate([p[0] + 2, p[1] + 2, -screw_boss_pad_h - 1])
                cylinder(d=shell_screw_clear_d, h=floor_t + screw_boss_pad_h + 2);
            translate([p[0] + 2, p[1] + 2, floor_t - shell_screw_csk_depth])
                cylinder(d=shell_screw_csk_d, h=shell_screw_csk_depth + 1);
        }
        
        translate([20.45 - 13.5, 7, -0.3]) //Right Lower
            linear_extrude(height = 2.5)
                circle(r = 3.2, $fn = 6);
        translate([44.45 + 13, 7, -0.3]) //Left Lower
            linear_extrude(height = 2.5)
                circle(r = 3.2, $fn = 6);
        translate([20.45 - 13.5, 92.4 - 0.5, -0.3]) //Right Upper
            linear_extrude(height = 2.5)
                circle(r = 3.2, $fn = 6);
        translate([44.45 + 13, 92.4 - 0.5, -0.3]) //Left Upper
            linear_extrude(height = 2.5)
                circle(r = 3.2, $fn = 6);
